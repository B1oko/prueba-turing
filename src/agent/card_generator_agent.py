...
class CardGeneratorAgent(BaseAgent):
    def __init__(self, llm: BaseChatModel, image_client: ImageGenerationClient):
        self._image_client = image_client
        super().__init__(llm)

    def _build_graph(self):
        designer_llm = self._llm.with_structured_output(CardSpecs)
        ...
        def generate_art(state: CardGeneratorState) -> dict:
            art_bytes = self._image_client.generate(state["card_specs"]["art_prompt"])
